...
class ManiSkillVectorEnvAlt(VectorEnv):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not self.ignore_terminations and auto_reset:
            assert self.base_env.reconfiguration_freq == 0 or self.base_env.num_envs == 1, "With partial resets, environment cannot be reconfigured automatically"

        if self.record_metrics:
            self.success_once = torch.zeros(self.num_envs, device=self.base_env.device, dtype=torch.bool)
            self.fail_once = torch.zeros(self.num_envs, device=self.base_env.device, dtype=torch.bool)
            self.returns = torch.zeros(self.num_envs, device=self.base_env.device, dtype=torch.float32)
